...
export async function generateScript(text: string, style: ScriptStyle): Promise<ScriptSegment[]> {
  const stylePrompt = STYLE_PROMPTS[style];
    ...
    5. **新增字段 1：AI 视频提示词 (videoPrompt)**。这是专门给 Sora/Kling/Luma 等工具使用的，必须严格遵循以下结构强制输出：
       **[主体具体描述] + [明确动作] + [具体场景环境] + [专业镜头语言/机位] + [光影/氛围关键词] + [风格/画质参数]**
       - 主体：性别、年龄、种族、服装、发型必须清晰可见。
       - 动作：必须是具体的走、跑、转头、凝视等，严禁模糊描述。
       - 场景：时间、光线、背景细节（如霓虹灯、雨后街道）。
       - 镜头：包含景别（近景/特写等）、运镜（手持跟随、环绕等）、机位高度。
       - 风格：电影级画质、真实写实风格、4K、高动态范围等。
